...
typedef enum _SYSTEM_INFORMATION_CLASS {
  ...
  SystemFileCacheInformation, // qs: SYSTEM_FILECACHE_INFORMATION; s (requires
  ...
  SystemInterruptInformation, // q: SYSTEM_INTERRUPT_INFORMATION (EX in: USHORT
                              // ProcessorGroup)
  SystemDpcBehaviorInformation,     // qs: SYSTEM_DPC_BEHAVIOR_INFORMATION; s:
                                    // SYSTEM_DPC_BEHAVIOR_INFORMATION (requires
                                    // SeLoadDriverPrivilege)
  SystemFullMemoryInformation,      // q: SYSTEM_MEMORY_USAGE_INFORMATION // not
                                    // implemented
  SystemLoadGdiDriverInformation,   // s: SYSTEM_GDI_DRIVER_INFORMATION
                                    // (kernel-mode only)
  SystemUnloadGdiDriverInformation, // s: SYSTEM_GDI_DRIVER_UNLOAD_INFORMATION
                                    // (kernel-mode only)
  SystemTimeAdjustmentInformation,  // qs: SYSTEM_QUERY_TIME_ADJUST_INFORMATION;
                                    // s: SYSTEM_SET_TIME_ADJUST_INFORMATION
                                    // (requires SeSystemtimePrivilege)
  SystemSummaryMemoryInformation,   // q: SYSTEM_MEMORY_USAGE_INFORMATION // not
                                    // implemented
  ...
  SystemRegistryQuotaInformation,    // qs: SYSTEM_REGISTRY_QUOTA_INFORMATION; s
  ...
  SystemVerifierInformation,   // qs: SYSTEM_VERIFIER_INFORMATION; s (requires
                               // SeDebugPrivilege)
  ...
  SystemWatchdogTimerHandler,     // s: SYSTEM_WATCHDOG_HANDLER_INFORMATION //
                                  // (kernel-mode only)
  SystemWatchdogTimerInformation, // qs: out: SYSTEM_WATCHDOG_TIMER_INFORMATION
                                  // (EX in: ULONG WATCHDOG_INFORMATION_CLASS)
  ...
  SystemSpecialPoolInformation, // qs: SYSTEM_SPECIAL_POOL_INFORMATION (requires
  ...
  SystemTimeZoneInformation,        // qs: RTL_TIME_ZONE_INFORMATION (requires
                                    // SeTimeZonePrivilege)
  ...
  SystemProcessorPerformanceDistribution, // q:
                                          ...
                                          // (EX in: USHORT ProcessorGroup) //
                                          // NtQuerySystemInformationEx // 100
  SystemNumaProximityNodeInformation, // qs: SYSTEM_NUMA_PROXIMITY_MAP
  SystemDynamicTimeZoneInformation,   // qs: RTL_DYNAMIC_TIME_ZONE_INFORMATION
  ...
  SystemRegistryAppendString, // s: SYSTEM_REGISTRY_APPEND_STRING_PARAMETERS //
                              // 110
  SystemAitSamplingValue, // s: SYSTEM_AIT_SAMPLING_VALUE_INFORMATION (requires
                          // SeProfileSingleProcessPrivilege)
  ...
  SystemEntropyInterruptTimingRawInformation, // qs:
                                              // SYSTEM_ENTROPY_TIMING_INFORMATION
  SystemPortableWorkspaceEfiLauncherInformation, // q:
                                                 // SYSTEM_PORTABLE_WORKSPACE_EFI_LAUNCHER_INFORMATION
  SystemFullProcessInformation, // q: SYSTEM_EXTENDED_PROCESS_INFORMATION with
                                // SYSTEM_PROCESS_INFORMATION_EXTENSION
                                // (requires admin)
  SystemKernelDebuggerInformationEx, // q: SYSTEM_KERNEL_DEBUGGER_INFORMATION_EX
  SystemBootMetadataInformation,     // q: SYSTEM_BOOT_METADATA_INFORMATION //
                                     // (requires SeTcbPrivilege) // 150
  SystemSoftRebootInformation, // qs: SYSTEM_SOFT_REBOOT_INFORMATION (s requires
  ...
  SystemVmGenerationCountInformation, // s: PHYSICAL_ADDRESS (kernel-mode only)
                                      // (vmgencounter.sys)
  ...
  SystemSecureDumpEncryptionInformation, // qs:
                                         ...
                                         // // NtQuerySystemInformationEx //
                                         // opaque dump-key transform buffer (q:
  ...
  SystemFirmwareBootPerformanceInformation, // q:
                                            ...
                                            // // HaliQuerySystemInformation ->
                                            // HalpFwBootPerformanceTable, info
                                            // class 34
  ...
  SystemEnclaveLaunchControlInformation, // q:
  ...
  SystemCodeIntegrityUnlockModeInformation, // q:
                                            // SYSTEM_CODEINTEGRITY_UNLOCK_INFORMATION
  SystemLeapSecondInformation,    // qs: SYSTEM_LEAP_SECOND_INFORMATION // (s:
                                  // requires SeSystemtimePrivilege)
  SystemFlags2Information,        // q: SYSTEM_FLAGS_INFORMATION // (s: requires
                                  // SeDebugPrivilege)
  ...
  SystemDifSetRuleClassInformation,  // s: SYSTEM_DIF_VOLATILE_INFORMATION
                                     // (requires SeDebugPrivilege)
  ...
  SystemBuildVersionInformation, // q: in:
                                 // SYSTEM_BUILD_VERSION_INFORMATION_INPUT, out:
                                 // SYSTEM_BUILD_VERSION_INFORMATION //
                                 // NtQuerySystemInformationEx //
                                 // CmQueryBuildVersionInformation
  SystemPoolLimitInformation,    // q: SYSTEM_POOL_LIMIT_INFORMATION (requires
                                 // SeIncreaseQuotaPrivilege) //
                                 // NtQuerySystemInformationEx
  ...
  SystemCodeIntegrityClearDynamicStores, // q:
                                         // CodeIntegrity-AllowConfigurablePolicy-CustomKernelSigners
  SystemDifPoolTrackingInformation, // s: SYSTEM_DIF_POOL_TRACKING_INFORMATION
                                    // (requires SeDebugPrivilege)
  ...
  SystemSecureKernelDebuggerInformation, // q: in:
...
typedef struct _SYSTEM_THREAD_INFORMATION {
  ...
  ULONG WaitTime; // The current time spent in ready queue or waiting (depending
                  // on the thread state).
  PVOID StartAddress; // The initial start address of the thread.
  CLIENT_ID ClientId; // The identifier of the thread and the process owning the
                      // thread.
...
typedef struct _SYSTEM_PROCESS_INFORMATION {
  ULONG NextEntryOffset; // The address of the previous item plus the value in
                         // the NextEntryOffset member. For the last item in the
  ...
  ULONGLONG WorkingSetPrivateSize; // The total private memory that a process
                                   // currently has allocated and is physically
  ...
  LARGE_INTEGER UserTime; // Number of 100-nanosecond intervals the process has
  ...
  HANDLE InheritedFromUniqueProcessId; // The identifier of the process that
                                       // created this process. Not updated and
                                       // incorrectly refers to processes with
                                       // recycled identifiers.
  ULONG HandleCount; // The current number of open handles used by the process.
  ULONG SessionId;   // The identifier of the Remote Desktop Services session
                     // under which the specified process is running.
  ULONG_PTR UniqueProcessKey; // since VISTA (requires
                              // SystemExtendedProcessInformation)
  SIZE_T PeakVirtualSize; // The peak size, in bytes, of the virtual memory used
                          // by the process.
  SIZE_T VirtualSize;   // The current size, in bytes, of virtual memory used by
                        // the process.
  ULONG PageFaultCount; // The total number of page faults for data that is not
                        // currently in memory. The value wraps around to zero
                        // on average 24 hours.
  ...
  SIZE_T QuotaPeakPagedPoolUsage; // The peak quota charged to the process for
                                  // pool usage, in bytes.
  SIZE_T QuotaPagedPoolUsage; // The quota charged to the process for paged pool
                              // usage, in bytes.
  SIZE_T QuotaPeakNonPagedPoolUsage; // The peak quota charged to the process
                                     // for nonpaged pool usage, in bytes.
  SIZE_T QuotaNonPagedPoolUsage; // The current quota charged to the process for
                                 // nonpaged pool usage.
  SIZE_T PagefileUsage; // The total number of bytes of page file storage in use
                        // by the process.
...
typedef enum _THREADINFOCLASS {
  ...
  ThreadManageWritesToExecutableMemory, // s: MANAGE_WRITES_TO_EXECUTABLE_MEMORY
                                        // // since REDSTONE3
